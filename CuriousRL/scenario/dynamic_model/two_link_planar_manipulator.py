...
class TwoLinkPlanarManipulator(DynamicModel):
    ...
    - xu6(action): Moment applied to the first joint :math:`\\tau_1`, limited to [-100, 100]
    - xu7(action): Moment applied to the second joint :math:`\\tau_2`, limited to [-100, 100]
    ...
    .. math::
        ...
        \\dot \\theta_2(k)+\\tau_2(k)\\\\
        \\ell_1\\sin\\big(\\theta_1(k)\\big)+\\ell_2\\sin\\big(\\theta_1(k)+\\theta_2(k)\\big)\\\\
        \\ell_1\\cos\\big(\\theta_1(k)\\big)+\\ell_2\\cos\\big(\\theta_1(k)+\\theta_2(k)\\big)
        \\end{bmatrix}
    ...
    def __init__(self, T = 100, x = 2, y = 2):
        self._action_space = ActionSpace(action_range=[[-100, 100], [-100, 100]], action_type=["Continuous", "Continuous"], action_info=["Moment 1", "Moment 2"])
        ##### Dynamic Function ########
        ...
        dynamic_function = sp.Array([  
            ...
            xu_var[2] + h*xu_var[3],
            xu_var[3] + h*xu_var[7],
            self.l1*sp.sin(xu_var[0]) + self.l2*sp.sin(xu_var[0]+xu_var[2]),
            self.l1*sp.cos(xu_var[0]) + self.l2*sp.cos(xu_var[0]+xu_var[2])])
