...
if __name__ == "__main__":
    logger.set_folder_name("VehicleTracking", remove_existing_folder=False).set_is_use_logger(True).set_is_save_json(True)
    scenario = VehicleTracking(T=100)
    # NNiLQR(gaussian_noise_sigma=[[0.01], [0.1]], iLQR_max_iter=10).init(scenario).solve()
    NNiLQR(gaussian_filter_sigma=10, gaussian_noise_sigma=[[0.01], [0.1]] ,training_stopping_criterion=1e-4, decay_rate=0.99, decay_rate_max_iters=300).init(scenario).solve()
    scenario.play("VehicleTracking")
# %%
